...
eloratingopt <- function(agofile, presencefile, sex, outputfile){
  # Get data
  ...
  # Format dates
  model_log$Date <<- strptime(as.character(model_log$Date), "%d.%m.%Y")
  model_log$Date <<- format(model_log$Date, "%Y-%m-%d")
  ...
  if(sex=="F"){
    ...
    df2$Date <-strptime(df2$Date, format="%Y-%m-%d")
    ...
    df2_daymax <- ddply(df2, .(Date, Individual), summarize, EloScoreAfterMax=max(EloScoreAfter))
    df2_daymax$Date <- strptime(df2_daymax$Date, format="%Y-%m-%d")
    # Split dataset by female ID
    ...
    presence_long$Date <- strptime(presence_long$Date, format="%d.%m.%Y")
    # Get complete list of dates from presence matrix and add scores for matching dates
    elobyid_full <- list()
    for(i in seq_along(elobyid_f)){
      elobyid_full[[i]] <- as.data.frame(presence[[1]])
      colnames(elobyid_full[[i]])[1] <- "Date"
      elobyid_full[[i]]$Date <- strptime(elobyid_full[[i]]$Date, format="%d.%m.%Y")
      # fill in ID info from first row of list object, for length of calendar
      elobyid_full[[i]]$Individual <- rep(elobyid_f[[i]][1,]$Individual, length(elobyid_full[[i]]$Date))
      # pull in elo scores based on date in elo list objects
      elobyid_full[[i]]$EloScoreAfter <- elobyid_f[[i]]$EloScoreAfterMax[match(elobyid_full[[i]]$Date, elobyid_f[[i]]$Date)]
    ...
    elo_data2 <- elo_data2[order(elo_data2$Date,elo_data2$Individual),]
    elo_matrix <<- t(matrix(elo_data2$EloScore, ncol=nrow(presence)))
    tail(elo_matrix)
    # elo_df <- as.data.frame(elo_matrix)
    # colnames(elo_df) <- inds
  ...
  rel_card_ranks_long$Date <- strptime(as.character(rel_card_ranks_long$Date), "%d.%m.%Y")
  ...
  # head(rank_data)
  if(sex=="F"){
    write.csv(rank_data, "rank_data_females.csv", row.names = F)
  } else if(sex=="M"){
    write.csv(rank_data, "rank_data_males.csv", row.names = F)
  }
}
