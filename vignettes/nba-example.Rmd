...
A familiar example may be useful for understanding the different fitting options available in the EloOptimized package.  We included in the package a set of game results from the 1995-1996 NBA regular season.  We used NBA data because games can't end in a tie, teams play all other teams over the course of the season, and there are a manageable set of teams for visualization purposes.  During the 1995-1996 season, the Chicago Bulls went 72-10 (until recently the best record in NBA history), had a winning record against all but one team, and went on to win the championship.  Therefore we would expect a reasonable ranking method to assign them the highest rank score by the end of the season.  First we'll use a traditional Elo ranking approach to assign Elo scores using `eloratingfixed()`, and then we'll show how maximum likelihood fitting improves things.

## Traditional Elo scores with default parameter values

First, we'll load the packages we'll need (we're using a couple tidyverse packages for post-fitting data manipulation and visualization):
...
```{r}
tail(res$elo, 29) %>% dplyr::arrange(-Elo) %>% select(-ExpNumBeaten, -JenksEloCardinal)
```
...
This takes longer than calculating traditional Elo scores because we're optimizing the k parameter before generating Elo scores.  It turns out that the optimized value of K is closer to 20, and this value improves our prediction accuracy to 0.663.  Let's take a look at our results:
...
```{r}
tail(res3$elo, 29) %>% dplyr::arrange(-Elo) %>% select(-ExpNumBeaten, -JenksEloCardinal)
```

Because each team plays the same number of games, and schedules are fairly balanced, the Elo scores are now just follow each team's overall record.  These scores are not useful as dynamic records of team performance, but would be useful for calculating winning probabilities according to the sinusoidal equation from [Foerster, Franz et al., 2016](https://www.nature.com/articles/srep35404).  And in an animal system where individuals enter and leave the hierarchy through maturation and death, respectively, and where individuals tend to enter towards the bottom of the hierarchy, such a result would reflect a queuing system, as argued in the linked article.  